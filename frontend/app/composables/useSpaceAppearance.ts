...
export function useSpaceAppearance() {
   ...
   *
   * `pet` is sent whenever the caller passes the key at all — including as `null`, which is how
   * the pet goes home. That's why it's an explicit `'pet' in patch` rather than a truthiness
   * check: "no pet" and "don't change the pet" are different requests.
   */
  async function save(patch: { avatar?: AvatarLook, pet?: PetKind | null }) {
